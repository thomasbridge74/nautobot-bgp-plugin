...
class RoutingPolicy(NetBoxModel):
    ...
    def get_absolute_url(self):
        return reverse("plugins:nautobot_bgp_plugin:routingpolicy", args=[self.pk])
...
class BGPPeerGroup(NetBoxModel):
    ...
    def get_absolute_url(self):
        return reverse("plugins:nautobot_bgp_plugin:bgppeergroup", args=[self.pk])
...
class BGPBase(NetBoxModel):
    ...
    status = models.CharField(
        max_length=50, choices=ASNStatusChoices, default=ASNStatusChoices.STATUS_ACTIVE
    )
...
class ASN(BGPBase):
    ...
    group = models.ForeignKey(ASNGroup, on_delete=models.PROTECT, blank=True, null=True)
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                fields=["number"], condition=models.Q(tenant=None), name="uniqie_number"
            ),
        ]
    ...
    def get_absolute_url(self):
        return reverse("plugins:nautobot_bgp_plugin:asn", args=[self.pk])
    ...
    def __str__(self):
        nb_settings = settings.PLUGINS_CONFIG.get("nautobot_bgp_plugin", {})
...
class Community(BGPBase):
    ...
    def get_absolute_url(self):
        return reverse("plugins:nautobot_bgp_plugin:community", args=[self.pk])
...
class BGPSession(NetBoxModel):
    ...
    local_as = models.ForeignKey(ASN, on_delete=models.PROTECT, related_name="local_as")
    remote_as = models.ForeignKey(
        ASN, on_delete=models.PROTECT, related_name="remote_as"
    )
    status = models.CharField(
        max_length=50,
    ...
    def get_absolute_url(self):
        return reverse("plugins:nautobot_bgp_plugin:bgpsession", args=[self.pk])
