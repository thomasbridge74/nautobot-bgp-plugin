...
class ASNFilterForm(NetBoxModelFilterSetForm):
    ...
    status = forms.MultipleChoiceField(
        choices=ASNStatusChoices, required=False, widget=StaticSelectMultiple()
    )
...
class ASNForm(NetBoxModelForm):
    number = ASNField(widget=ASdotInput)
    tags = DynamicModelMultipleChoiceField(queryset=Tag.objects.all(), required=False)
    site = DynamicModelChoiceField(queryset=Site.objects.all(), required=False)
    status = forms.ChoiceField(
        required=False, choices=ASNStatusChoices, widget=StaticSelect()
    )
    tenant = DynamicModelChoiceField(queryset=Tenant.objects.all(), required=False)
        ...
        if "number" in self.changed_data or "tenant" in self.changed_data:
            if ASN.objects.filter(number=number, tenant=tenant).exists():
                raise forms.ValidationError(
                    "AS number with this number and tenant is already exists."
                )
        return cleaned_data
...
class ASNBulkEditForm(NetBoxModelBulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=ASN.objects.all(), widget=forms.MultipleHiddenInput
    )
    tenant = DynamicModelChoiceField(queryset=Tenant.objects.all(), required=False)
    ...
    model = ASN
    fieldsets = ((None, ("tenant", "description", "status")),)
    nullable_fields = (
        "tenant",
        "description",
    )
