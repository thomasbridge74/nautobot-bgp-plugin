...
class ASNFilterForm(BootstrapMixin, CustomFieldModelForm):
    ...
    status = forms.MultipleChoiceField(
        choices=ASNStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
...
class ASNBulkEditForm(BootstrapMixin, CustomFieldBulkEditForm):
    ...
    status = forms.ChoiceField(
        required=False, choices=ASNStatusChoices, widget=StaticSelect2()
    )
...
class BGPSessionFilterForm(BootstrapMixin, CustomFieldModelForm):
    ...
    status = forms.MultipleChoiceField(
        choices=SessionStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
